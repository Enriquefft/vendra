...
import { scenarioConfigSchema, type ScenarioConfigInput } from "@/lib/schemas/session";

import { ScenarioConfigForm } from "./scenario-config-form";
import { initialCreateSessionState, type CreateSessionState } from "./types";
...
async function createSessionAction(
        ...
        const scenarioConfig: ScenarioConfigInput = {
                ...
                contactType: (formData.get("contactType") ?? "cold_call") as ScenarioConfigInput["contactType"],
                ...
                simulationPreferences: {
                        ...
                        clientIntensity: (formData.get("clientIntensity") ?? "neutro") as ScenarioConfigInput["simulationPreferences"]["clientIntensity"],
                        maxDurationMinutes: Number(formData.get("maxDurationMinutes") ?? 0),
                        realism: (formData.get("realism") ?? "humano") as ScenarioConfigInput["simulationPreferences"]["realism"],
                },
                targetProfile: {
        ...
        try {
                ...
                if (!response.ok) {
                        ...
                        return {
                                message: errorBody?.error ?? "No se pudo crear la sesión de simulación.",
                                status: "error",
                        };
                }

                const result = (await response.json()) as { persona: PersonaProfile; sessionId: string };
        ...
        return (
                <main className="mx-auto flex min-h-screen max-w-6xl flex-col gap-10 px-6 py-12">
                        <section className="rounded-3xl bg-gradient-to-br from-blue-900 via-slate-900 to-slate-950 p-10 text-white shadow-2xl">
                                <p className="text-sm uppercase tracking-[0.25em] text-blue-200">Configuración</p>
                                <h1 className="mt-2 text-4xl font-bold">Define tu escenario</h1>
                                <p className="mt-3 max-w-3xl text-lg text-slate-200">
                                        Completa los detalles del producto, el perfil objetivo y las preferencias de simulación. Generaremos un cliente realista vinculado a tu sesión.
                                </p>
                        </section>

                        <ScenarioConfigForm action={createSessionAction} initialState={initialCreateSessionState} />
                </main>
        );
}

...
mock.module("@/components/audio-recorder-button", () => ({
	AudioRecorderButton: ({
		...
		<button
			type="button"
			disabled={disabled}
			onClick={() => onTranscription("Mensaje de vendedor")}
		>
			Hablar con cliente
		</button>
	),
...
describe("User flows", () => {
	test("Flow 1: configure a scenario and see generated persona", async () => {
		...
		const createSessionAction = async (_state: unknown, formData: FormData) => {
			...
			expect(formData.get("productName")).toBe(
		...
		await user.type(
			screen.getByLabelText(/Descripción breve/i),
			scenarioConfigFixture.description,
		);
		await user.type(
			screen.getByLabelText(/Objetivo de la llamada/i),
			scenarioConfigFixture.callObjective,
		);
		await user.type(
			screen.getByLabelText(/Rango de edad/i),
			scenarioConfigFixture.targetProfile.ageRange,
		);
		await user.type(
			screen.getByLabelText(/Ubicación/i),
			scenarioConfigFixture.targetProfile.location,
		);
		await user.type(
	...
	test("Flow 2: send a message in simulation and finish the call", async () => {
		...
		render(
			<SimulationView
				...
				initialMessages={[]}
				maxDurationMinutes={
					scenarioConfigFixture.simulationPreferences.maxDurationMinutes
				}
			/>,
		);

		await user.click(
			screen.getByRole("button", { name: /Hablar con cliente/i }),
		);
